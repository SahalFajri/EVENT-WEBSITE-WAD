  ...
  <div class="max-w-4xl mx-auto p-8 bg-white shadow-lg rounded-lg">
    ...
    <form action="{{ route('dashboard.ticket.update', $ticket->id) }}" method="POST">
      ...
      <div class="mb-5">
        ...
        <input type="number" id="stock" name="stock" value="{{ old('stock', $ticket->stock) }}"
          class="w-full px-4 py-2 text-sm text-gray-800 bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
      ...
      <div class="mb-5">
        ...
        <textarea id="description" name="description" rows="4"
          class="w-full px-4 py-2 text-sm text-gray-800 bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
          placeholder="Enter ticket description" required>{{ old('description', $ticket->description) }}</textarea>
        @error('description')
          <span class="text-sm text-red-600">{{ $message }}</span>
        @enderror
      </div>
      ...
      <div class="mb-5">
        ...
        <select name="is_available" id="is_available"
          class="w-full px-4 py-2 text-sm text-gray-800 bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
          required>
          <option value="0" @if ($ticket->is_available === 0) selected @endif>No</option>
          <option value="1" @if ($ticket->is_available === 1) selected @endif>Yes</option>
        </select>
        @error('is_available')
          <span class="text-sm text-red-600">{{ $message }}</span>
        @enderror
